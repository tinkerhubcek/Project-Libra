...
class BookForm(forms.ModelForm):
    ...
    Code = forms.ModelChoiceField(queryset=BarCode.objects.all(
    ...
    category = forms.ChoiceField(widget=forms.Select(
        attrs={"class": "btn btn-primary dropdown-toggle"}), choices=CATEGORIES)
...
class LogForm(forms.ModelForm):
    ...
    bar_code_no = forms.ModelChoiceField(queryset=BarCode.objects.all(
    ), widget=forms.Select(attrs={"class": "btn btn-primary dropdown-toggle"}))
    book = forms.ModelChoiceField(queryset=Book.objects.all(), widget=forms.Select(
        attrs={"class": "btn btn-primary dropdown-toggle"}), required=True)
    borrower = forms.ModelChoiceField(queryset=Log_user.objects.all(
    ), widget=forms.Select(attrs={"class": "btn btn-primary dropdown-toggle"}))
    status = forms.ChoiceField(widget=forms.Select(
        attrs={"class": "btn btn-primary dropdown-toggle"}), choices=STATUS)
